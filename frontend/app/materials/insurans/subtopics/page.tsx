...
function getStepTone(type: (typeof INSURANS_STEPS)[number]["type"]) {
...
function stepTypeLabel(type: (typeof INSURANS_STEPS)[number]["type"]) {
  if (type === "Content") return "Kandungan";
  if (type === "Exercise") return "Latihan";
  return "Pentaksiran";
}

export default function InsuransSubtopicsPage() {
  const router = useRouter();
  const [completedIds, setCompletedIds] = useState<string[]>([]);
  const [currentSubtopicId, setCurrentSubtopicId] = useState(INSURANS_SUBTOPICS[0].id);
  ...
  useEffect(() => {
    ...
    const updateCurrentSubtopic = () => {
      ...
      const closestStep = INSURANS_STEPS[activeIndex];
  ...
  const currentSubtopic = useMemo(
    () => INSURANS_SUBTOPICS.find((subtopic) => subtopic.id === currentSubtopicId) ?? INSURANS_SUBTOPICS[0],
    [currentSubtopicId]
  );
  ...
  function isUnlocked(index: number) {
    ...
    const previousStep = INSURANS_STEPS[index - 1];
    return completedIds.includes(previousStep.id);
  }

  function handleCircleClick(step: (typeof INSURANS_STEPS)[number], index: number) {
    if (!isUnlocked(index) && !isCompleted(step.id)) return;
    router.push(`/materials/insurans/subtopics/${step.id}`);
  }
