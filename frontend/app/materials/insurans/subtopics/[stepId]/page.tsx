...
export default function InsuransStepPage() {
  ...
  const step = useMemo(
    () => INSURANS_STEPS.find((item) => item.id === params.stepId) ?? null,
    [params.stepId]
  );

  const subtopic = useMemo(() => {
    if (!step) return null;
    return INSURANS_SUBTOPICS.find((item) => item.id === step.subtopicId) ?? null;
  }, [step]);
