...
function getStepTone(type: (typeof UBAHAN_STEPS)[number]["type"]) {
...
export default function UbahanSubtopicsPage() {
  ...
  const [currentSubtopicId, setCurrentSubtopicId] = useState(UBAHAN_SUBTOPICS[0].id);
  ...
  useEffect(() => {
    ...
    const updateCurrentSubtopic = () => {
      ...
      const closestStep = UBAHAN_STEPS[activeIndex];
  ...
  function isUnlocked(index: number) {
    ...
    const previousStep = UBAHAN_STEPS[index - 1];
    return completedIds.includes(previousStep.id);
  }

  function handleCircleClick(step: (typeof UBAHAN_STEPS)[number], index: number) {
    if (!isUnlocked(index) && !isCompleted(step.id)) return;
    router.push(`/materials/ubahan/subtopics/${step.id}`);
  }
